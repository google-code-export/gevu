...
var xscale = d3.scale.ordinal().rangePoints([0, w], 1),
    yscale = {};
	yticks = {"Logement":1,"Nombre_pieces":2,"Etage": (d3.time.scale(0, 3, 1)) ,"Surface_Reelle":20,"Date_Construction": d3.time.years(1900, 2012, [10]) , "Consommation_Reelle":500};
	
	//D'apr�s https://github.com/mbostock/d3/wiki/Time-Formatting
	
	//linear.invert(y) --> Retourne la valeur dans le domaine d'entr�e x pour la valeur correspondante dans la plage de sortie y .
	//linear.domain([numbers]) --> Si le nombre est sp�cifi�, �tablit domaine d'entr�e de l'�chelle dans le tableau sp�cifi� de chiffres
	//linear.range([values]) --> Si les valeurs est sp�cifi�e, d�finit la plage de sortie de l'�chelle � l'ensemble des valeurs sp�cifi�es.	
	// linear.rangeRound(values) --> D�finit la plage de sortie de l'�chelle dans le tableau sp�cifi� de valeurs, tout en �tablissant interpolateur l'�chelle de d3.interpolateRound .
	//linear.interpolate([factory]) --> Si l'usine est sp�cifi�, d�finit interpolateur sortie de la balance � l'aide du sp�cifi�es en usine .
	//linear.clamp([boolean]) --> Si bool�en est sp�cifi�, active ou d�sactive serrage en cons�quence. 
	//linear.nice() --> �tend le domaine de sorte qu'il commence et se termine sur Nice valeurs arrondies. 
	// linear.ticks(count) --> Retours d'environ compter des valeurs repr�sentatives de domaine d'entr�e de l'�chelle
	// linear.tickFormat(count) --> Retourne un format de num�ro de fonction appropri�e pour afficher une valeur ticks.
	//linear.copy() --> Retourne une copie exacte de cette �chelle lin�aire.
	//(d3.time.year, 300)
	// (d3.time.format (Y%))
