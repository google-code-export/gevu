...
function optimiser_base($attente = 86400) {
	...
	$res = spip_query("SELECT mots_syndic.id_mot,mots_syndic.id_syndic
	...
	$res = spip_query("SELECT auteurs_messages.id_auteur,auteurs_messages.id_message
	...
	$res = spip_query("SELECT auteurs_rubriques.id_auteur,auteurs_rubriques.id_rubrique
		      FROM spip_auteurs_rubriques AS auteurs_rubriques
		        LEFT JOIN spip_rubriques AS rubriques
		          ON auteurs_rubriques.id_rubrique=rubriques.id_rubrique
		       WHERE rubriques.id_rubrique IS NULL");
	...
	$result = spip_query("DELETE FROM spip_mots
	...
	$res = spip_query("SELECT mots_forum.id_mot,mots_forum.id_forum
