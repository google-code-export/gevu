...
/*** hadi updLieuxHierarchie(1, 10, 0, 1, 0); */
...
$dbO = "gevu_global";
migreBase($dbN, $dbO);      /*** was commented ***/
ajusteBase(1);              /*** was commented ***/
...
function ModifBranche($r1, $oR, $idInstant){
	...
	//les �l�ments li� au lieu ayant le m�me nom que la r�f�rence
	if($r1['nbLieux']>0){
		...
		//on r�cup�re les enfants de ref�rence et ceux � comparer pour le niveau suivant
		$rs2 = mysql_query($sql);
	...
	//si seulement l'identifiant de lieu est diff�rent
	//on met � jour les �l�ments li�s 
...
function ChangeBranche($id_lieuSrc, $lftDst, $idInstantDst, $niv=0){
	...
	*/
	...
	while($r=mysql_fetch_array($result)){
		//on ajoute la branche � la r�f�rence
...
function migreBase($dbN, $dbO){
        ...
        /** ajout de hadi
        **/
        // chercher un lft=0 (chercher univers). Si n'existe pas, cr�er ce noeud de base (cr�er univers)
        ...
        //r�cup�re les propri�t� de l'univers
...
function setLieuxHierarchie($idParent, $dbN, $dbO, $idInstant, $idRub, $lft, $rgt, $niv=1){
   ...
   while($row=mysql_fetch_array($res)) {
	...
	//on insert un lieu pour r�cup�rer l'identifiant
   	$sql = "INSERT INTO $dbN.gevu_lieux
...
//pour mettre � jour la hi�rarchie des lieux
...
function ajusteBase($idLieu){
	...
	//r�cup�re les lieux avec le m�me nom, niveau, id_parent, id_rubrique
	$sql = "SELECT count(*) nb, GROUP_CONCAT(l.id_lieu) ids
		...
		ORDER BY l.maj DESC";
	$rs = mysql_query($sql);
	if (!$rs) echo 'Requ�te invalide : ' . mysql_error().'<br/>'.$sql.'<br/>';
	
	while($r=mysql_fetch_array($rs)) {
		CompareBranche($r['ids'], $idInstant, $idLieu);
	}

	//r�cup�re les lieux du niveau inf�rieur pour changer leur instant et r�calculer les gauche droite
	...
	//on met � jour la droite de l'univers
	$sql = "SELECT MAX(rgt)+1 m FROM gevu_lieux";
	$result = mysql_query($sql);
	$r=mysql_fetch_array($result);
	$sql = "UPDATE gevu_lieux SET rgt=".$r['m']." WHERE lft = 0";
...
function CompareBranche($idsLieux, $idInstant, $LieuParent){
	...
	//r�cup�re les liens du lieu ayant la m�me rubrique et le m�me parent 
	...
	while($r1=mysql_fetch_array($rs1)) {
		if($oLib!=$r1['lib']){
			...
			*/	
			//le lieux le plus r�cent avec un nom diff�rent sert de r�f�rence
			//on met � jour l'instant de la r�f�rence et de ses enfants
			//ChangeBranche($r1['id_lieu'], $r1['lft'], $r1['rgt'], $r1['id_instant'], $Plft, $idInstant, $r1['niv']);							
			$oLieu = $r1['id_lieu'];
			$oLft = $r1['lft'];
			$oK = $r1['k'];
			$oR = $r1;
			//on met � jour les instants de la branche
			...
			//on v�rifie si les lieux sont identiques
			if($oDate==$r1['maj'] && $oK==$r1['k'] ){
				//on supprime le lieu et tout ces �lements
				DeleteLieu($r1['id_lieu'], $r1['lft'], $r1['rgt'], $r1['id_instant']);
			//on v�rifie s'il y a des �l�ments li�s
			}elseif($r1['nbTot']==0){
				//on supprime la donn�e
				DeleteLieu($r1['id_lieu'], $r1['lft'], $r1['rgt'], $r1['id_instant']);
			}else{
				ModifBranche($r1, $oR, $idInstant);
				//on met � jour le parent
