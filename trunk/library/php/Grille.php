				...
				if(!$xml){
					...
					while($row = mysql_fetch_assoc($rs)) {
						...
						if(count($Q)>0){ 					
							...
							{
								$arrSG = $grille->FiltreRubAvecGrilleMultiSite($row["id_rubrique"],$grilleEnf["idForm"],$GetAll);
								ksort($arrSG);
								$i=0;
								$xmlSG=""; 					
				...
				switch ($id) {
					...
					default:
						...
						if($idDon && $id!=$this->site->infos["GRILLE_ACTEUR"]){
							...
							$carto = "";
							$AddGeo ="<button label='Ajouter une g�olocalisation' oncommand=\"AddPlacemark(".$r["idRub"].",'".$this->type."');\"/>";
						}
