...
/* charge le tagcloud dès le load
d3.select(window)
    .on("load", hashchange)
...
function parseText(text) {
  ...
  text.split(wordSeparators).forEach(function(word) {
	...
	//on supprime les élisions
    var i = word.search(self.elision);
	if(i>0)word = word.substr(i+1);
    //on supprime les numériques
    if(!isNaN(parseFloat(word)) && isFinite(word)) return;	
...
function draw(data, bounds) {
  ...
  text.enter().append("text")
      ...
      .attr("transform", function(d) { return "translate(" + [d.x, d.y] + ")rotate(" + d.rotate + ")"; })
      .style("font-size", function(d) { return d.size + "px"; })
      //aucune action sur le click .on("click", function(d) {load(d.text);})
      .style("opacity", 1e-6)
    .transition()
      .duration(1000)
      .style("opacity", 1);
