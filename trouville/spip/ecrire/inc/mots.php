...
function affiche_mots_ressemblant($cherche_mot, $objet, $id_objet, $resultat, $table, $table_id, $url_base)
{
	...
	while ($row = spip_fetch_array($result)) {
		...
		$type_mot = pipeline('arbo_groupe',array('args'=>array('exec'=>'mots_edit','id_groupe'=>$row['id_groupe']),'data'=>''));
...
function afficher_mots_cles($flag_editable, $objet, $id_objet, $table, $table_id, $url_base, $visible)
{
	...
	// On recupere le typo_mot ici, et non dans le mot-cle lui-meme; sinon bug avec arabe
			...
			$type_mot = pipeline('arbo_groupe',array('args'=>array('exec'=>'mots_edit','id_groupe'=>$id_groupe),'data'=>''));
			//fin ajout Lucky Semiosis
...
function formulaire_mots_cles($id_groupes_vus, $id_objet, $les_mots, $table, $table_id, $url_base, $visible, $objet) {
	...
	$result_groupes = spip_query("SELECT id_groupe,unseul,obligatoire,titre, ".creer_objet_multi ("titre", $spip_lang)." FROM spip_groupes_mots WHERE $table = 'oui' AND ".substr($connect_statut,1)." = 'oui' AND (unseul != 'oui'  OR (unseul = 'oui' AND id_groupe NOT IN ($cond_id_groupes_vus))) ORDER BY id_groupe, multi");
...
function menu_mots($row, $id_groupes_vus, $les_mots)
{
	...
	$result = spip_query("SELECT id_mot, type, titre FROM spip_mots WHERE id_groupe =$id_groupe " . ($les_mots ? "AND id_mot NOT IN ($les_mots) " : '') .  "ORDER BY id_groupe, titre");
	...
	$titre = pipeline('arbo_groupe',array('args'=>array('exec'=>'mots_edit','id_groupe'=>$id_groupe),'data'=>''));
	//fin ajout Lucky Semiosis
	...
	if ($n > 50) {
		...
		if ($obligatoire)
			$res .= "<input type='text' name='cherche_mot' class='fondl' style='width: 280px; background-color:#E86519;' value=\"$titre_groupe\" size='180' $jscript>";
		else if ($unseul)
			$res .= "<input type='text' name='cherche_mot' class='fondl' style='width: 280px; background-color:#cccccc;' value=\"$titre_groupe\" size='180' $jscript>";
		else
			$res .= "<input type='text' name='cherche_mot'  class='fondl' style='width: 280px; ' value=\"$titre_groupe\" size='80' $jscript>";
		...
		if ($obligatoire)
			$res .= "<SELECT NAME='nouv_mot' SIZE='1' STYLE='width: 280px; background-color:#E86519;' CLASS='fondl' $jscript>";
		else if ($unseul)
			$res .= "<SELECT NAME='nouv_mot' SIZE='1' STYLE='width: 280px; background-color:#cccccc;' CLASS='fondl' $jscript>";
		else
			$res .= "<SELECT NAME='nouv_mot' SIZE='1' STYLE='width: 280px; ' CLASS='fondl' $jscript>";
